//! `omapac-repo attest` and `index` over a fake repository directory, with
...
fn attest_then_index_round_trip() {
    ...
    let index_bytes = std::fs::read(repo.join("omapac-index.json")).unwrap();
    let signature = std::fs::read_to_string(repo.join("omapac-index.json.minisig")).unwrap();
    ...
    let index: serde_json::Value =
        serde_json::from_slice(&std::fs::read(repo.join("omapac-index.json")).unwrap()).unwrap();
    assert_eq!(
