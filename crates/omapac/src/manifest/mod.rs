...
//! `omapac.toml` files, with the managed floor applied last. See
//! `PLAN.md`, "Manifest, lockfile, ledger, managed config".
//!
//! Layers, lowest to highest: `/etc/omapac/omapac.toml`,
//! `/etc/omapac/conf.d/*.toml` in name order, then the user's
//! `$XDG_CONFIG_HOME/omapac/omapac.toml`. The same package key wins by
//! last layer. `/etc/omapac/managed.toml` and `$OMAPAC_MANAGED_CONFIG_PATH`
...
impl ManifestPaths {
    ...
    pub fn conventional(sysroot: Option<&Path>) -> ManifestPaths {
        ...
        let mut managed = vec![rooted("/etc/omapac/managed.toml")];
        if let Some(extra) = std::env::var_os("OMAPAC_MANAGED_CONFIG_PATH") {
            managed.push(PathBuf::from(extra));
        }
        ManifestPaths {
            system: rooted("/etc/omapac/omapac.toml"),
            conf_d: rooted("/etc/omapac/conf.d"),
            user: config_home.join("omapac/omapac.toml"),
            managed,
        }
    }
...
mod tests {
    ...
    fn paths(dir: &Path) -> ManifestPaths {
        ManifestPaths {
            system: dir.join("etc/omapac/omapac.toml"),
            conf_d: dir.join("etc/omapac/conf.d"),
            user: dir.join("home/.config/omapac/omapac.toml"),
            managed: vec![dir.join("etc/omapac/managed.toml")],
        }
    }
