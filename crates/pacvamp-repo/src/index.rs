//! `pacvamp-repo index`: the signed index of a repository directory. See
...
pub const INDEX_FILE: &str = "pacvamp-index.json";
...
/// accepted build keys, and writes pacvamp-index.json plus its minisign
...
impl RunWith<()> for IndexCmd {
    ...
    fn run_with(self, _: ()) -> Self::Output {
        ...
        let signature = key
            .sign(
                &bytes,
                &format!("pacvamp-index {} sequence {}", index.repo, index.sequence),
            )
            .to_file();
