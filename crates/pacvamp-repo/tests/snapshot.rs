...
impl Train {
    ...
    fn run(&self, now: &str, args: &[&str]) -> (i32, String, String) {
        ...
        self.rig.run_env(&full, &[("PACVAMP_REPO_NOW", now)])
    }

    fn release(&self, id: &str) -> pacvamp::channel::Release {
        let path = self
            .rig
            .path()
...
fn cut_test_promote_hold_prune() {
    ...
    assert_eq!(
        ...
        pacvamp::channel::TestResult::Fail
    );
    ...
    let (code, out, _) = t.run(
        ...
        &[
            ...
            "2026-09-02T06",
            "--suite",
            "echo tested: hyprland; test -n \"$PACVAMP_SNAPSHOT_DIR\"; exit 1",
        ],
    );
    assert_ne!(code, 0);
    ...
    let (code, out, err) = t.run(
        ...
        &["test", "--id", "2026-09-02T06", "--suite", "echo suite-log; echo suite-error >&2; echo tested: hyprland; echo tested: omarchy; echo tested: hyprland; test \"$PACVAMP_SNAPSHOT_ID\" = 2026-09-02T06", "--commit", "abc", "--log-url", "https://ci/1"],
    );
    assert_eq!(code, 0, "{err}");
