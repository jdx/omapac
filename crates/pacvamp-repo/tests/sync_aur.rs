...
fn rpc() -> String {
    let info = std::fs::read_to_string(
        Path::new(env!("CARGO_MANIFEST_DIR")).join("../pacvamp/fixtures/aur/info.json"),
    )
    .unwrap();
...
fn low_reputation_rpc() -> String {
    ...
    let yay = info["results"]
        ...
        .unwrap();
    yay["Maintainer"] = "new-maintainer".into();
    yay["Submitter"] = "original-submitter".into();
    yay["NumVotes"] = 0.into();
...
impl Gate {
    ...
    fn run(&self, extra: &[&str]) -> (i32, String, String) {
        ...
        self.rig.run_env(
            &args,
            &[
                ("PACVAMP_AUR_RPC_BASE", &self.rpc),
                ("PACVAMP_AUR_GIT_BASE", &self.aur.base()),
                ("PACVAMP_REPO_NOW", "2026-09-03T00:00:00Z"),
            ],
        )
    }
...
fn clean_bump_by_trusted_maintainer_auto_merges() {
    ...
    packslip::minisign::SecretKey::from_seed([5u8; 32])
        ...
        .unwrap();
    let feed: pacvamp::trust::feeds::Verdicts = serde_json::from_slice(&feed_bytes).unwrap();
    ...
    assert_eq!(verdict.verdict, pacvamp::trust::feeds::VerdictKind::Pass);
    ...
    assert_eq!(code, 0);
    assert!(out.contains("unchanged    yay"), "{out}");
    let feed: pacvamp::trust::feeds::Verdicts =
...
fn hostile_takeover_is_blocked_with_a_block_verdict() {
    ...
    assert_eq!(verdict.verdict, pacvamp::trust::feeds::VerdictKind::Block);
    assert!(
...
fn verdict_and_advisory_feeds() {
    ...
    packslip::minisign::SecretKey::from_seed([5u8; 32])
        ...
        .verify(&feed_bytes, &sig)
        .unwrap();
    let feed: pacvamp::trust::feeds::Advisories = serde_json::from_slice(&feed_bytes).unwrap();
    assert_eq!(feed.advisories[0].commits, vec!["3f9c1a2b"]);
    assert_eq!(
        feed.advisories[0].action,
        pacvamp::trust::feeds::AdvisoryAction::Block
    );
