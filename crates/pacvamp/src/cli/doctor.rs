...
fn diagnose_host(host: &Host, add: &mut impl FnMut(Status, &str, String)) {
    ...
    for source in &host.sources {
        ...
        match source.db_modified() {
            None => add(
                ...
                "sync",
                format!(
                    "[{}] has no sync database yet; run `pacvamp update` or `pacman -Sy`",
                    source.name
                ),
            ),
            Some(modified) => {
    ...
    match crate::trust::Keyring::load(host.paths.sysroot.as_deref()) {
        Ok(keyring) if keyring.is_empty() => add(
            Status::Warn,
            "trust",
            "no pacvamp trust keys under /etc/pacvamp/keys or /usr/share/pacvamp/keys; repository feeds cannot be verified".to_string(),
        ),
        Ok(keyring) => add(
            Status::Ok,
            "trust",
