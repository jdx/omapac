...
fn require_pacvamp_index(source: &crate::host::Source, target: &str) -> Result<()> {
    if !matches!(source.tier, Tier::Opr | Tier::Custom(_)) {
        bail!(
            "{target} comes from [{}] ({}), which publishes no pacvamp index; pacman's signature check is its evidence",
            source.name,
            source.tier
        );
    }
        ...
        let (name, repo, filename, file): (String, String, String, Option<PathBuf>) =
            if explicit_path && path.is_file() {
                ...
                for source in &host.sources {
                    ...
                    {
                        require_pacvamp_index(source, &filename)?;
                ...
                require_pacvamp_index(source, &self.target)?;
                let cached = host
                    .config
                    .options
                    .cache_dirs()
