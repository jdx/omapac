...
//! libalpm guard hook aborts any `-Su` without it; pacvamp is the sanctioned
//! updater.
...
impl PacmanCli {
    ...
    pub fn detect() -> Result<PacmanCli> {
        ...
        if let Some(pacman) = std::env::var_os("PACVAMP_TEST_PACMAN") {
        ...
        let pacman = if system.is_file() {
            ...
            which::which("pacman").map_err(|_| {
                Error::NotAvailable("pacman is not on PATH; pacvamp needs pacman".to_string())
            })?
        };
    ...
    fn refresh_and_file_install_argv() {
        ...
        let install = FileInstall {
            files: vec![PathBuf::from(
                "/home/u/.cache/pacvamp/aur/foo/foo-1.0-1-x86_64.pkg.tar.zst",
            )],
            as_deps: true,
            overwrite: vec![],
        };
        assert_eq!(
            engine
                ...
                .display(),
            "/usr/bin/pacman --config /tmp/pacman.conf --sysroot /mnt -U --noconfirm --asdeps -- /home/u/.cache/pacvamp/aur/foo/foo-1.0-1-x86_64.pkg.tar.zst"
        );
    }
