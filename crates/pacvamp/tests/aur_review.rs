...
fn review_approve_then_catch_the_takeover() {
    ...
    let lock_path = s.rig.home.join(".config/pacvamp/pacvamp.lock");
...
fn split_package_approval_is_keyed_by_pkgbase() {
    ...
    assert_eq!(code, 0, "{err}");
    let lock =
        std::fs::read_to_string(setup.rig.home.join(".config/pacvamp/pacvamp.lock")).unwrap();
...
fn feeds_shape_the_review() {
    ...
    let verdicts = format!(
        r#"{{"version":1,"sequence":1,"issued_at":"2026-09-03T00:00:00Z","verdicts":[
          ...
          {{"subject":{{"pkgbase":"yay","commit":"{head}"}},"reviewer":{{"kind":"static","id":"pacvamp-policy"}},"verdict":"flag","summary":"odd source","issued_at":"2026-09-03T00:00:00Z"}}]}}"#
    );
    ...
    assert!(
        out.contains("DENY  verdict: static reviewer pacvamp-policy says Flag: odd source"),
        "{out}"
    );
    assert!(
    ...
    s.rig.write_root("/etc/pacman.conf", &conf);
    std::fs::remove_dir_all(s.rig.dir.path().join("cache/pacvamp/trust")).ok();
