//! The ledger records what pacvamp did, and `list --ledger` / `--drift`
//! read it back.
...
fn install_records_and_remove_forgets() {
    let rig = Rig::new();
    let ledger = rig.root.join("var/lib/pacvamp/state.json");
    rig.write_root(
    ...
    assert!(
        out.contains("curl") && out.contains("removed outside pacvamp"),
        "{out}"
    );
...
fn as_deps_records_named_targets_as_dependencies() {
    ...
    let state: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(rig.root.join("var/lib/pacvamp/state.json")).unwrap(),
    )
    .unwrap();
...
fn ledger_and_drift_views() {
    let rig = Rig::new();
    // Pretend pacvamp installed pacman at an older version than the fixture
...
fn a_newer_schema_is_refused() {
    let rig = Rig::new();
    rig.write_root("/var/lib/pacvamp/state.json", r#"{"schema":9}"#);
...
fn hidden_ledger_merge_reads_a_patch_from_stdin() {
    ...
    let state = std::fs::read_to_string(rig.root.join("var/lib/pacvamp/state.json")).unwrap();
