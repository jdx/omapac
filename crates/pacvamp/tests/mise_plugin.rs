...
#[test]
fn plugin_lists_and_installs_through_pacvamp() {
    ...
    let pacvamp_dir = Path::new(env!("CARGO_BIN_EXE_pacvamp"))
        .parent()
        .unwrap()
        .to_path_buf();
    let mise = Mise {
        ...
        home,
        project,
        path: format!("{}:/usr/bin:/bin", pacvamp_dir.display()),
    };
    let plugin = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../plugins/mise-tool-channel");
    let channel_env = [
        ("PACVAMP_TOOLS_BASE", store.base.as_str()),
        ("PACVAMP_TOOLS_PUBKEY", store.channel_pub.to_str().unwrap()),
    ];
    ...
    edge.push(("PACVAMP_TOOLS_CHANNEL", "edge"));
    ...
    // A held version is refused by pacvamp, so mise reports the failure.
    let (code, out, err) = mise.run(
