...
fn pacvamp(root: &Path, args: &[&str]) -> (i32, String, String) {
...
fn present_and_missing() {
    let root = sysroot();
    let root = root.path();
    assert_eq!(pacvamp(root, &["present", "pacman"]).0, 0);
    assert_eq!(pacvamp(root, &["present", "pacman>=7", "glibc"]).0, 0);
    assert_eq!(
        pacvamp(root, &["present", "libalpm.so=16-64"]).0,
        0,
        "by provision"
    );
    assert_eq!(pacvamp(root, &["present", "pacman", "helix"]).0, 1);
    assert_eq!(pacvamp(root, &["present", "pacman<7"]).0, 1);
    assert_eq!(pacvamp(root, &["missing", "helix", "zathura"]).0, 0);
    assert_eq!(pacvamp(root, &["missing", "helix", "pacman"]).0, 1);
}

#[test]
fn list_with_tiers_and_filters() {
    let root = sysroot();
    let (code, out, _) = pacvamp(root.path(), &["list"]);
    assert_eq!(code, 0);
    insta::assert_snapshot!(out);

    let (_, out, _) = pacvamp(root.path(), &["list", "--explicit"]);
    assert_eq!(out.lines().count(), 1);
    assert!(out.starts_with("pacman"));

    let (_, out, _) = pacvamp(root.path(), &["list", "--orphans"]);
    // glibc is a dependency that pacman depends on; yay is a dependency
    // nothing needs.
    assert_eq!(out.lines().count(), 1, "{out}");
    assert!(out.starts_with("yay"));

    let (_, out, _) = pacvamp(root.path(), &["list", "--foreign"]);
    assert!(out.is_empty(), "yay is carried by [omarchy]: {out}");

    let (_, out, _) = pacvamp(root.path(), &["list", "--json", "--native"]);
...
fn search_by_terms() {
    ...
    let (_, out, _) = pacvamp(root.path(), &["search", "--installed", "pacman"]);
    assert!(
        out.lines()
...
fn info_for_sync_and_installed_packages() {
    let root = sysroot();
    let (code, out, _) = pacvamp(root.path(), &["info", "pacman"]);
    assert_eq!(code, 0);
    ...
    let (_, out, _) = pacvamp(root.path(), &["info", "--json", "yay"]);
    ...
    let (_, out, _) = pacvamp(root.path(), &["info", "--json", "--no-aur", "yay"]);
    let infos: Vec<serde_json::Value> = serde_json::from_str(&out).unwrap();
...
fn doctor_reports_signature_floor_and_missing_databases() {
    let root = sysroot();
    let (code, out, _) = pacvamp(root.path(), &["doctor"]);
    ...
    let (_, out, _) = pacvamp(root.path(), &["doctor", "--json"]);
