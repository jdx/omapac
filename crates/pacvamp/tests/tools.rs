//! `pacvamp tools` against a hand-built channel store served locally: the
...
use pacvamp::trust::tools::{ToolArtifact, ToolEntry, ToolIndex, ToolVersion};
...
fn index_and_list() {
    ...
    assert!(
        out.contains("tool: 3 version(s) from github.com/example/tool"),
        "{out}"
    );
    ...
    let cache_key = pacvamp::trust::sha256_bytes(s.base.trim_end_matches('/').as_bytes());
    let sysroot_key = pacvamp::trust::sha256_bytes(s.rig.root.as_os_str().as_encoded_bytes());
    std::fs::write(
        s.rig.dir.path().join(format!(
            "cache/pacvamp/trust/sysroot-{}/tools/{cache_key}/index.sequence",
            &sysroot_key[..32]
        )),
        "9",
    )
