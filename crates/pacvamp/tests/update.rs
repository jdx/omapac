//! `pacvamp update` through the fake pacman, makepkg, AUR remote, and RPC.
...
fn setup(rpc_body: String) -> Setup {
    ...
    std::fs::create_dir_all(rig.home.join(".config/pacvamp")).unwrap();
    std::fs::write(
        rig.home.join(".config/pacvamp/pacvamp.toml"),
        "[policy]\naur.jail = false\n",
    )
    .unwrap();
    Setup { rig, aur, rpc }
...
fn post_hooks_run_after_an_apply_failure() {
    let s = setup(INFO.to_string());
    s.rig.write_root(
        "/etc/pacvamp/conf.d/10-hooks.toml",
        "[update]\npre_hooks = [\"echo hook-pre >> $FAKE_PACMAN_LOG\"]\npost_hooks = [\"echo hook-post >> $FAKE_PACMAN_LOG\"]\n",
    );
...
fn core_index(key: &packslip::minisign::SecretKey, sequence: u64, at: &str) -> String {
    ...
    common::http::serve(vec![
        ("/core/os/x86_64/pacvamp-index.json.minisig", sig),
        ("/core/os/x86_64/pacvamp-index.json", body),
    ])
}
...
fn age_floor_prefers_the_index_publish_time() {
    ...
    let recent = jiff::Timestamp::from_second(pacvamp::ledger::now() - 86_400)
        .unwrap()
        .to_string();
    for (published, floor, expect_hold) in [
        ...
        let s = setup(INFO.to_string());
        s.rig
            .write_root("/etc/pacvamp/keys/omarchy.pub", &key.public_key().to_file());
        ...
        assert_eq!(code, 0, "{err}\n{out}");
        assert!(
            !s.rig.root.join("var/lib/pacvamp/state.json").exists(),
...
fn only_one_update_runs_at_a_time() {
    ...
    let lock = s.rig.root.join("var/lib/pacvamp/update.lock");
    ...
    let (code, _, err) = run(&s, &["update", "-n", "--no-aur"], UPGRADE);
    assert_ne!(code, 0);
    assert!(err.contains("another pacvamp update is running"), "{err}");
